...
use async_std::{sync::RwLock, task, net::TcpStream};
use ruisutil::ArcMut;

use crate::{
    app::Application,
    entity::{conf::ProxyInfoConf, proxy::{ProxyListRep, ProxyGoto}},
    utils, engine::proxyer::Proxyer,
};
...
impl ProxyEngine {
    ...
    pub async fn goto(&self,data: ProxyGoto,conn:TcpStream) -> io::Result<()> {
      match self.inner.node.find_node(&data.proxy_host).await {
        Err(e) => {
        ...
        Ok(v) => {
            let connlc = match v
                .wait_conn(&data.localhost, data.proxy_port)
                .await
            {
                Ok(v) => v,
                Err(e) => {
